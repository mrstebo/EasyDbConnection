...
namespace EasyDbConnection.Tests.Commands
{
    [TestFixture]
    [Parallelizable]
    public class ExecuteScalarCommandTests
    {
        [SetUp]
        public void SetUp()
        {
            _connection = A.Fake<IDbConnection>();
            _command = new ExecuteScalarCommand(_connection);
        }

        private IDbConnection _connection;
        private ExecuteScalarCommand _command;
        ...
        public void Execute_runs_a_DbCommand()
        {
            ...
            A.CallTo(() => _connection.CreateCommand())
                .Returns(dbCommand);
            A.CallTo(() => dbCommand.ExecuteScalar())
                .Returns("my-value");
